...
from src.utils.formatters import create_progress_bar, format_duration


# ── YouTube-branded color ──────────────────────────────
YT_COLOR = discord.Color.from_str('#FF0000')
YT_ICON = 'https://i.imgur.com/OMBfeLU.png'


def build_yt_np_embed(song, paused=False, repeat=REPEAT_OFF, audio_filter='Normal',
                       autoplay=False, volume=0.5, current_time=0):
    status = '⏸️ Đang tạm dừng' if paused else '🎬 Đang phát (YouTube)'
    embed = discord.Embed(
        title=song.title,
        url=song.url,
        color=YT_COLOR if not paused else discord.Color.greyple(),
    )
    embed.set_author(name=status, icon_url=YT_ICON)
    if song.thumbnail:
    ...
    embed.set_footer(text=f'Yêu cầu bởi {song.requester.display_name} • 📥 Phát từ file local',
                     icon_url=song.requester.display_avatar.url)
    return embed


class YTControllerView(discord.ui.View):
    def __init__(self, player):
        super().__init__(timeout=None)
        self.player = player

    @discord.ui.button(emoji='⏸️', style=discord.ButtonStyle.secondary, custom_id='yt_pause')
    ...
    @discord.ui.button(emoji='⏭️', style=discord.ButtonStyle.primary, custom_id='yt_skip')
    async def skip(self, interaction: discord.Interaction, button: discord.ui.Button):
        vc = interaction.guild.voice_client
        if vc and (vc.is_playing() or vc.is_paused()):
            from src.core.config import REPEAT_OFF
    ...
    @discord.ui.button(emoji='⏹️', style=discord.ButtonStyle.danger, custom_id='yt_stop')
    ...
    @discord.ui.button(emoji='🔀', style=discord.ButtonStyle.secondary, custom_id='yt_shuffle')
    ...
    @discord.ui.button(emoji='🔁', style=discord.ButtonStyle.secondary, custom_id='yt_repeat')
    async def repeat(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.player.repeat_mode = (self.player.repeat_mode + 1) % 3
        await self._update_np(interaction)

    @discord.ui.button(emoji='✨', style=discord.ButtonStyle.secondary, custom_id='yt_autoplay')
    async def autoplay(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.player.autoplay = not self.player.autoplay
        await self._update_np(interaction)

    @discord.ui.button(emoji='🔉', style=discord.ButtonStyle.secondary, custom_id='yt_voldown')
    ...
    @discord.ui.button(emoji='🔊', style=discord.ButtonStyle.secondary, custom_id='yt_volup')
    ...
    async def _update_np(self, interaction):
        ...
        embed = build_yt_np_embed(
            self.player.current, paused=paused, repeat=self.player.repeat_mode,
            audio_filter=self.player.filter_name, autoplay=self.player.autoplay,
            volume=self.player.volume, current_time=self.player.get_current_time()
        )
        await interaction.response.edit_message(embed=embed, view=self)


class YTSearchSelect(discord.ui.Select):
    """Dropdown showing top 5 YouTube search results."""
    ...
    def __init__(self, results, requester):
        ...
        for i, r in enumerate(results[:5]):
            ...
            dur_str = format_duration(dur) if dur else 'Live'
        ...
        super().__init__(placeholder='🎬 Chọn bài hát từ YouTube...', options=options)

    async def callback(self, interaction: discord.Interaction):
        from .downloader import YTSongInfo
        idx = int(self.values[0])
        chosen = self.results[idx]
        cog = interaction.client.get_cog('YouTube')
        if not cog:
            return

        player = cog.get_player(interaction)
        title = chosen.get('title', 'Unknown')
        await interaction.response.edit_message(content=f'⏳ **Đang thêm:** {title}...', view=None)

        # Create song info — if flat result, do full extraction for reliable metadata
        url = chosen.get('url') or f"https://www.youtube.com/watch?v={chosen.get('id', '')}"
        try:
            song = await YTSongInfo.from_url(url, requester=self.requester, loop=interaction.client.loop)
        except Exception:
            # Fallback: use flat data directly
            song = YTSongInfo(chosen, self.requester)
        ...
        await interaction.edit_original_response(
            content=f'✅ Đã thêm: **{song.title}** `[{song.duration_str}]` 📥'
        )
...
class YTSearchView(discord.ui.View):
    def __init__(self, results, requester):
        ...
        self.add_item(YTSearchSelect(results, requester))
