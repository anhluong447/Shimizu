...
class MusicPlayer:
    """Class quản lý nhạc cho từng Server (Guild)"""
    ...
    async def player_loop(self):
        """Vòng lặp phát nhạc liên tục từ hàng đợi"""
        ...
        while not self.bot.is_closed():
            ...
            try:
                # Đợi bài tiếp theo trong hàng đợi
                async with asyncio.timeout(300):  # 5 phút không có nhạc thì tự thoát
            ...
            # Dọn dẹp sau khi bài kết thúc
            source.cleanup()
            self.current = None

    def destroy(self, guild):
        """Dọn dẹp và thoát khỏi phòng voice"""
...
class Music(commands.Cog):
    ...
    def get_player(self, ctx):
        """Lấy player hiện tại của server hoặc tạo mới"""
        try:
    ...
    async def play_(self, ctx, *, search: str):
        """Phát nhạc hoặc tìm kiếm từ khóa"""
        vc = ctx.voice_client
        if not vc:
    ...
    async def pause(self, ctx):
        ...
        if not ctx.voice_client or not ctx.voice_client.is_playing():
            return await ctx.send("Có đang phát gì đâu mà dừng bồ?")
    ...
    async def resume(self, ctx):
        ...
        if not ctx.voice_client or not ctx.voice_client.is_paused():
            return await ctx.send("Nhạc có đang tạm dừng đâu?")
    ...
    async def now_playing(self, ctx):
        ...
        await ctx.send(f"🎵 **Đang phát:** `{player.current.title}`")
...
@bot.event
async def on_ready():
    print(f'✅ Bot nhạc Pro đã sẵn sàng: {bot.user.name}')
